...
export function activate(context: vscode.ExtensionContext) {
  console.log('test-gen extension is now active');

  // Register command to generate tests for current file
  const generateTestsDisposable = vscode.commands.registerCommand('test-gen.generateTests', async () => {
    try {
  ...
  const generateTestsForDirectoryDisposable = vscode.commands.registerCommand('test-gen.generateTestsForDirectory', async (resource: vscode.Uri) => {
    try {
      if (!resource) {
  ...
  const setAIProviderDisposable = vscode.commands.registerCommand('test-gen.setAIProvider', async () => {
    ...
    if (selected) {
      const config = vscode.workspace.getConfiguration('test-gen');
